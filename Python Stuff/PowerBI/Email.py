...
os.chdir(r'C:\Users\F068039\OneDrive - Westpac Group\Upload EWAS RWA')
...
with teradatasql.connect(host='tdp5dbcw.teradata.westpac.com.au', user='XXXXX', password='XXXX') as con:
    with con.cursor () as cur:
        t1 = pd.read_sql('select * from bpabba1.ewas_digital_leads_raw',con)

t1['ReceivedTime'] = pd.to_datetime(t1['ReceivedTime'],utc=True)

start = t1['ReceivedTime'].max() + pd.Timedelta(minutes=1)
